...
public sealed class PeerBlacklist
{
    ...
    public PeerBlacklist(AppDatabase appDatabase)
    {
        _db = appDatabase ?? throw new ArgumentNullException(nameof(appDatabase));
    }
    ...
    public async Task<IReadOnlyList<PeerBlacklistEntity>> ListAsync(int userId, CancellationToken cancellationToken = default)
    {
        ...
        return await _db.ReadAsync(async conn =>
        {
            var rows = await conn.Table<PeerBlacklistEntity>()
                ...
                .ToListAsync()
                .ConfigureAwait(false);
            return (IReadOnlyList<PeerBlacklistEntity>)rows.OrderByDescending(r => r.AddedUtcTicks).ToList();
        }).ConfigureAwait(false);
    }
}
