...
public sealed class SqliteBleDiscoveredPeerStore(AppDatabase db) : IBleDiscoveredPeerStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            if (bluetoothMac.Data.Length != BluetoothTransportAddress.MacLength)
                return;
            mac = BluetoothTransportAddress.ToMacString(bluetoothMac.Data);
        }
        else if (bluetoothMac.Kind == TransportKind.WifiDirect)
        {
            if (!WifiDirectTransportAddress.TryParseAddress(bluetoothMac.Data, out var deviceId))
                return;
            mac = deviceId;
        }
        else
        ...
        string? idShort = null;
        if (scanResult.NetworkId is { } networkId && !networkId.IsEmpty)
    ...
    public async ValueTask RecordPingAsync(TransportAddress bluetoothMac, CompressedNetworkId peerNetworkId,
        string nickname,
        CancellationToken cancellationToken = default)
    {
        if (bluetoothMac.Kind != TransportKind.Bluetooth ||
