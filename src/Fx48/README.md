# ShortP2P .NET Framework 4.7.2 libraries

Separate `net472` assemblies that compile the existing ShortP2P sources (plus a few compat shims).
They are for the Win7/WinForms client: **HTTPS messenger servers, local SQLite, UDP LAN scan, QR from image file**. No BLE, no camera.
